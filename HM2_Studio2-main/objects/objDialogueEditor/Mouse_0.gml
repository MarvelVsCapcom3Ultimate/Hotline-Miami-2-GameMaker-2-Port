...
if (mouse_x>x and mouse_x<x+width-12 and mouse_y>=y+28 and mouse_y<=y+height) or (choice==0 and mouse_y>=y+28) {
if choice=1 nothing=1 else {
choice=0
select=startpos+floor(((mouse_y-(y+28))*(1/(height/(maxvalues+1)))))
if select>ds_list_size(searchlist)-1 select=ds_list_size(searchlist)
write=0
}
}
