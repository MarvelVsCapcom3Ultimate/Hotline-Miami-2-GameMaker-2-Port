function scrSoldierRandomStep() {
	...
	if seen=0 alert=1
	if seen=2 alert=2
	if seen==1 and alert>0 {if alert<3 {alert=5 chasereload=0}}
	lookreload=15
	}
	if instance_exists(objPlayerDead) {alert=0 path_end() speed=sign(speed)}
	if alert<2 and alert>0 {path_end() scrCharge(objPlayer.x,objPlayer.y,10,4)}
	if alert=2 {path_end() speed=0 scrEnemyAim(objPlayer.x,objPlayer.y,10,4)}
	if alert=3 and path_index<=0 alert=4
	if alert>=4 {if chasereload>0 chasereload-=1 else {chasereload=20 if alert>=4 scrChase(path,objPlayer.x,objPlayer.y) if alert>3 alert-=1}}
