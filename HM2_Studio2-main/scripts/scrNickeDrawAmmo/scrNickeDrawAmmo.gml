function scrNickeDrawAmmo() {
	if room=rmTutorialFloor1 or room=rmTutorialFloor2 or room=rmTutorialFloor2MovieSet or room=rmFansHeadQuarters or room=rmTalkShow or instance_exists(objMaskMenu) or instance_exists(objConversation) nothing=1 else {
	scrGetAmmoDraw()
	if global.ammotype=2 global.magammotype=5 else global.magammotype=4
	if global.ammo>0 ammostring=string(global.ammo) else {if global.magammo=0 ammostring="Empty" else ammostring=string(global.ammo)}
	...
	draw_set_font(fntAmmo)
	if global.ammo+global.magammo>0 draw_sprite_ext(sprAmmoSymbols,global.ammotype,20+ammox,232,1,1,0,c_white,1)
	if global.magammo>0 draw_sprite_ext(sprAmmoSymbols,global.magammotype,80+ammox,232,1,1,0,c_white,1)
	magammostring=string(global.magammo)
	if string_length(magammostring)=1 magammostring="0"+string(global.magammo)
	//ammo
	draw_text(31+ammox+lengthdir_x(0.5,dir),219+lengthdir_x(0.5,dir),string_hash_to_newline(ammostring))
	draw_text(35+ammox,223,string_hash_to_newline(ammostring))
	draw_text_color(34+ammox,222,string_hash_to_newline(ammostring),merge_color(c_fuchsia,c_purple,0.5+lengthdir_x(0.5,dir)),merge_color(c_fuchsia,c_purple,0.5+lengthdir_x(0.5,dir)),merge_color(c_red,c_maroon,0.5+lengthdir_x(0.5,dir)),merge_color(c_red,c_maroon,0.5+lengthdir_x(0.5,dir)),1)
	draw_text_color(32+ammox+lengthdir_x(0.5,dir),220+lengthdir_x(0.5,dir),string_hash_to_newline(ammostring),c_white,c_white,c_aqua,c_aqua,1)
	//magazine ammo
	if global.magammo>0 {
	draw_text(91+ammox+lengthdir_x(0.5,dir),219+lengthdir_x(0.5,dir),string_hash_to_newline(magammostring))
	draw_text(95+ammox,223,string_hash_to_newline(magammostring))
	draw_text_color(94+ammox,222,string_hash_to_newline(magammostring),merge_color(c_fuchsia,c_purple,0.5+lengthdir_x(0.5,dir)),merge_color(c_fuchsia,c_purple,0.5+lengthdir_x(0.5,dir)),merge_color(c_red,c_maroon,0.5+lengthdir_x(0.5,dir)),merge_color(c_red,c_maroon,0.5+lengthdir_x(0.5,dir)),1)
	draw_text_color(92+ammox+lengthdir_x(0.5,dir),220+lengthdir_x(0.5,dir),string_hash_to_newline(magammostring),c_white,c_white,c_aqua,c_aqua,1)
	}
	draw_set_font(fntCombo)
