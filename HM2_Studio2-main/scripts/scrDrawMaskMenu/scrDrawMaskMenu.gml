function scrDrawMaskMenu() {
	...
	if on=1 {
	if alpha<0.9 alpha+=0.1 else alpha=1
	i=0
	repeat (8) {
	...
	if alpha>0.1 alpha-=0.1 else alpha=0
	i=0
	repeat (8) {
	...
	draw_sprite_ext(sprMasksBig,i,__view_get( e__VW.WView, 0 )/2+addx+i*80,__view_get( e__VW.HView, 0 )/2,scale,scale,lengthdir_x(5,dir+i*45),color,1)
	i+=1
	}
